...
def xy_horizontalhistogram(
    x_list, y_list, xValue, title
):  # inputs 2 lists and returns histogram
    fig, ax = plt.subplots()
    # data
    ...
    ax.set(xlim=[0, 12], xlabel=xValue, ylabel="", title=title)
    plt.tight_layout()
...
def generate_network(
    FAAM,
):  # returns a Networkx graph given the FAAM elements dictionary
    net = nx.Graph()
    # node attributes
    nodes_colors = {
    ...
    for manifestation in FAAM["manifestations"]:
        url = (
            "<a href='"
            + manifestation["FAAMUrl"]
            + ">"
            + manifestation["title"]
            + ", "
            + manifestation["FAAM-ID"]
            + "</a>"
        )
        title = "<body>" + url + "</body>"
        add_node(
            ...
            manifestation["FAAM-ID"],
            manifestation["title"],
            nodes_colors["manifestation"],
            10 * len(manifestation["musicalWorks"]),
            title,
        )
    # add work nodes
...
def inputCsv2faamJson(
    manifestations_dict, works_dict, annotations_dict
):  # Cleans-up, organizes and collates duplicates
    # Initialize main FAAM categories
    FAAM = {"manifestations": [], "agents": [], "musicalWorks": [], "annotations": []}
    ...
    for row in manifestations_dict["items"]:
        ...
        try:
            ...
            if (
                row["FAAM-ID"] == current_manifestation["FAAM-ID"]
            ):  # do not create new manifestation keep adding metadata
                if any(
                    x.get("workName") == row["Musical Works"]
                    for x in current_manifestation["musicalWorks"]
                ):
                    pass
                else:
                    current_manifestation["musicalWorks"].append(
                        {"workName": row["Musical Works"]}
                    )
                if any(
                    x.get("agentName") == row["[Agent] Agent"]
                    for x in current_manifestation["agents"]
                ):
                    pass
                else:
                ...
                if any(
                    x.get("annotationName") == row["Editorial annotations"]
                    for x in current_manifestation["annotations"]
                ):
                    pass
                else:
                    current_manifestation["annotations"].append(
                        {"annotationName": row["Editorial annotations"]}
                    )  # current manifestation case
    ...
    for row in works_dict["items"]:
        try:
            query = list(
                filter(
                    lambda x: x[1].get("workName") == row["\ufeffName"],
                    enumerate(FAAM["musicalWorks"]),
                )
            )
            current_work = FAAM["musicalWorks"][query[0][0]]
            # add arrangers
            if any(
                x.get("agentName") == row["arranger"] for x in current_work["arrangers"]
            ):
                pass
            else:
                # append new arranger
                current_work["arrangers"].append(
                    {"agentName": row["arranger"], "qid": row["arranger QID"]}
                )
            ...
            FAAM["musicalWorks"].append(
                {
                    ...
                    "composers": [
                        {"agentName": row["composer"], "qid": row["composer QID"]}
                    ],
                    "arrangers": [
                        {"agentName": row["arranger"], "qid": row["arranger QID"]}
                    ],
    ...
    for row in annotations_dict["items"]:
        try:
            query = list(
                filter(
                    lambda x: x[1].get("annotationName") == row["\ufeffName"],
                    enumerate(FAAM["annotations"]),
                )
            )
            ...
            if len(query) > 0:
                # add subclass
                if any(
                    x.get("annotationName") == row["subclass of"]
                    for x in current_annotation["subclass_of"]
                ):
                    pass
                else:
                    # append subclass
                ...
                if any(
                    x.get("annotationName") == row["musical parameter"]
                    for x in current_annotation["musical_parameters"]
                ):
                    pass
                else:
...
def faaamStatistics(FAAM):  # Generates statistics for FAAM JSON
    ...
    for manifestation in FAAM["manifestations"]:
        for musicalWork in manifestation["musicalWorks"]:  # add manif. to musical works
            if musicalWork["workName"] == "":
                ...
                query = list(
                    filter(
                        lambda x: x[1].get("workName") == musicalWork["workName"],
                        enumerate(FAAM["musicalWorks"]),
                    )
                )
                if len(query) > 0:
                    # update occurrence
                    FAAM["musicalWorks"][query[0][0]]["FAAM-IDs"].append(
                        manifestation["FAAM-ID"]
                    )
                else:
        ...
        for agent in manifestation["agents"]:  # add agent to agents list
            query = list(
                filter(
                    lambda x: x.get("agentName") == agent["agentName"], FAAM["agents"]
                )
            )
            if len(query) > 0:  # agent exists in list
                ...
                FAAM["agents"].append(
                    {"agentName": agent["agentName"], "agentRole": agent["agentRole"]}
                )
        for annotation in manifestation[
            "annotations"
        ]:  # add annotations to annotations list
            query = list(
                filter(
                    lambda x: x[1].get("annotationName")
    ...
    for work in FAAM["musicalWorks"]:  # add qids to agents and annotations
        for composer in work["composers"]:
            query = list(
                filter(
                    lambda x: x[1].get("agentName") == composer["agentName"],
                    enumerate(FAAM["agents"]),
                )
            )
            if len(query) > 0:
        ...
        for arranger in work["arrangers"]:
            if arranger["agentName"] != "":
                query = list(
                    filter(
                        lambda x: x[1].get("agentName") == arranger["agentName"],
                        enumerate(FAAM["agents"]),
                    )
                )
                if len(query) > 0:
        ...
        for faamId in work["FAAM-IDs"]:
            ...
            query = list(
                filter(
                    lambda x: x[1].get("FAAM-ID") == faamId,
                    enumerate(FAAM["manifestations"]),
                )
            )
            for annotation in FAAM["manifestations"][query[0][0]]["annotations"]:
                query = list(
                    filter(
                        lambda x: x.get("annotationName")
                        == annotation["annotationName"],
                        work["annotations"],
                    )
                )
    ...
    for work in FAAM["musicalWorks"]:
        if work["occurrence"] >= max_occurrence:
            high_occurrences.append(
                {"workName": work["workName"], "occurrence": work["occurrence"]}
            )
...
input_annotations_csv_filename = (
    "../../FAAM_annotations/FAAM-annotations_export-" + date + ".csv"
)
...
x_list = []
y_list = []
sortedHighOccurrenceWorks = sorted(
    FAAM["statistics"]["highOccurrenceWorks"], key=lambda x: x["occurrence"]
)
...
xy_horizontalhistogram(
    x_list[-12:], y_list[-12:], "Occurence", "Works with high occurrence"
)
